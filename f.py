...
def get_image_to_tensor():
    images = glob.glob("face/*jpg")
    image = random.choice(images)
    print(image)
    img = Image.open(image)
    trans = transforms.ToPILImage()
    trans1 = transforms.ToTensor()
    test = trans1(img).view(trans1(img).size(0), -1)
    return test
...
for epoch in range(2):
    for i in range(500):
        data = get_image_to_tensor()# trainloader에는 mnist가 들어있는데, data에 그것을 계속 보내준다 i에는 걍 숫자가 있다
        X = data # X에는 숫자의 배열(mnist의 그것)이 들어가고 _에는 인덱스가 있다.

        # print(X[1])
        # mb_size = X.size(0)     #64를 가짐
        # X = X.view(X.size(0),-1)    #그걸 평평하게 함 28*28 이 784가 됨
        #
        one_labels = torch.ones(256,1)
        zero_labels = torch.zeros(256,1)
        #
        z = torch.randn(784)  # 64*100의 크기 ,노이즈임
